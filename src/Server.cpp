...
// Possible probleme: si dans le premier echange de donnee/buffer il n'y a pas de password le client sera kick
...
void Server::authentication(int fd, const char *buffer) {
	...
	if (!getClientByFd(fd).isAuth()) {
		for (int i = 0; i < parser.message.size(); i++) {
			if (parser.message[i][0] == "PASS" && parser.message[i].size() > 0) {
				if (parser.message[i][0 + 1] == this->_password) {
